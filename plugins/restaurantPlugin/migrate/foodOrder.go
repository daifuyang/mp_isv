...
func (migrate *FoodOrder) AutoMigrate() {
	...
	db.AutoMigrate(&model.FoodOrderRefund{})
	db.AutoMigrate(&FoodOrderDetail{})

	prefix := cmf.Conf().Database.Prefix

	db.Exec("drop event if exists orderCloseStatus")
	db.Exec("drop event if exists orderFinishStatus")
	...
	db.Exec("UPDATE cmf_food_order SET refund_fee = fee WHERE refund_fee = 0 AND (order_status !=  'TRADE_CLOSED'' OR order_status != 'TRADE_CLOSED')")
