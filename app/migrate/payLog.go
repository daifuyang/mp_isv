...
type PayLog struct {
	...
	GmtPayment     int64   `gorm:"type:bigint(20);index:idx_gmt_payment;comment:交易付款时间" json:"gmt_payment"`
...
func (m PayLog) AutoMigrate() {

	flag := cmf.NewDb().Migrator().HasIndex(&PayLog{}, "gmt_payment")
	if flag {
		cmf.NewDb().Migrator().CreateIndex(&PayLog{}, "gmt_payment")
	}
	flag = cmf.NewDb().Migrator().HasIndex(&PayLog{}, "idx_gmt_payment")
	if flag {
		cmf.NewDb().Migrator().CreateIndex(&PayLog{}, "idx_gmt_payment")
	}
