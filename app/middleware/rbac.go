	...
	if tx.RowsAffected > 0 {
		for _,ruleItem := range ruleApi{
			...
			if ruleItem.Param != "" {
				...
				if !b {
					new(controller.RestController).ErrorCode(c, 10002, "您无权访问该页面！", nil)
					c.Abort()
					return
				}
		...
		// 查询当前角色是否进行授权
		tx := cmf.NewDb().Debug().Table(prefix+"auth_access access").Select("access.*,r.name").
			Joins("INNER JOIN "+prefix+"auth_rule r ON access.rule_name = r.name").Where(queryStr, queryArgs...).Scan(&authAccessRule)

		if tx.Error != nil {
			new(controller.RestController).Error(c,  tx.Error.Error(), nil)
			c.Abort()
			return
		}
